...
class Image
{
     ...
     * @throws Exception('upload_error', $this->_fileReference['error'])
     */
     ...
     * @throws Exception('resample_error')
     */
    public function resize($imageWidth, $imageHeight, $ratio = FALSE, $width = TRUE)
    {
        ...
        if (!$bool) {
            throw new Exception('resample_error');
        }
     ...
     * @throws Exception unknown_file_type, save_error
     */
    public function save($destination, $type, $extension = FALSE)
    {
        ...
        switch ($type) {
            ...
            default:
                throw new Exception('unknown_file_type', $type);
                break;
        }

        if (!$bool) {
            throw new Exception('save_error', $destination);
        }
    }
     ...
     * @throws Exception tmp_file_open_error
     */
    public function binary($type, $slashes = FALSE)
    {
        ...
        $this->save(CORE_TEMP, $type, 'temp_image' . $this->_time);
        $f = @fopen(CORE_TEMP . 'temp_image' . $this->_time, 'r');

        if (!$f) {
            throw new Exception(
                'tmp_file_open_error',
                CORE_TEMP . 'temp_image' . $this->_time
            );
        }

        $arr['size']    = @filesize(CORE_TEMP . 'temp_image'.$this->_time);
     ...
     * 
     * @throws Exception invalid_extension, create_error, tmp_file_open_error
     */
        ...
        switch ($type) {
            ...
            case "binary":
                ...
                if ($this->_imageHandler) {
                    $this->_checkTempDirectory();
                    $f = @fopen(
                        CORE_TEMP . 'temp_image' . $this->_time, 'r'
                    );
            ...
            default:
                throw new Exception('invalid_extension', $type);
                break;
        }

        if (!$this->_imageHandler){
            throw new Exception('create_error', $type);
        }
